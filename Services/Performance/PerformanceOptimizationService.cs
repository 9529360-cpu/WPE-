...
public class PerformanceOptimizationService : IDisposable
{
    ...
    {
        _dataProcessor = new RealtimeDataProcessor(batchSize: 100, maxQueueSize: 10000, parallelism: 4);
        _cacheManager = new SmartCacheManager(defaultExpiration: TimeSpan.FromMinutes(15), maxCacheSize: 10000);
    ...
    /// 获取数据处理统计
    /// </summary>
    public PerformanceStats GetDataProcessorStats() => _dataProcessor.GetStats();
        ...
        TimeSpan? expiration = null) where T : class
    {
        ...
        {
            T value = await _cacheManager.GetOrCreateAsync(key, factory, expiration);
            if (value is null)
    ...
    /// 删除缓存数据
    /// </summary>
    public void RemoveCached(string key) => _cacheManager.Remove(key);
    ...
    /// 获取热点键
    /// </summary>
    public System.Collections.Generic.List<HotKeyInfo> GetHotKeys(int topN = 20) => _cacheManager.GetHotKeys(topN);
    ...
    /// 记录操作
    /// </summary>
    public IDisposable RecordOperation(string operationName) => _performanceMonitor.RecordOperation(operationName);
    /// <summary>
    /// 记录自定义指标
    /// </summary>
    public void RecordMetric(string name, double value) => _performanceMonitor.RecordMetric(name, value);
    /// <summary>
    /// 获取系统指标
    /// </summary>
    public SystemMetrics GetSystemMetrics() => _performanceMonitor.GetSystemMetrics();
    /// <summary>
    /// 获取请求统计
    /// </summary>
    public RequestStatistics GetRequestStatistics() => _performanceMonitor.GetRequestStatistics();
    /// <summary>
    /// 获取完整性能报告
    /// </summary>
    public PerformanceReport GetPerformanceReport() => _performanceMonitor.GetReport();
    /// <summary>
    /// 打印性能报告
    /// </summary>
    public void PrintPerformanceReport() => _performanceMonitor.PrintReport();
...
public class HealthCheckResult
{
    ...
    public override string ToString()
        => $"{(IsHealthy ? "✅ 健康" : "⚠️ 异常")} - CPU: {CpuUsage:F2}%, Memory: {MemoryUsageMB}MB, CacheHit: {CacheHitRate:P2}, DropRate: {DataDropRate:P2}";
}
