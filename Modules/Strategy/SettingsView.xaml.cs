...
public partial class SettingsView : UserControl
{
    ...
    private readonly ObservableCollection<AutoTradeStatus> _statuses = new();
    private readonly AutoTradeEngine _autoTrade = ServiceLocator.AutoTrade;
    ...
    public SettingsView()
    {
        ...
        foreach (var status in _autoTrade.GetStatuses())
            _statuses.Add(status);
        _autoTrade.StatusChanged += AutoTrade_StatusChanged;
    ...
    private async void StartAutoTrade_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            var config = BuildConfig();
            ...
            if (!config.EnableAutoTrade)
                throw new InvalidOperationException("请先勾选 \"启用自动交易\" 复选框");

            StatusText.Text = $"状态：启动 {config.StrategyName} 自动交易中...";
            await _autoTrade.StartStrategyAsync(config);
            StatusText.Text = $"状态：策略 {config.StrategyName} 已进入自动交易";
        }
        catch (Exception ex)
        {
            StatusText.Text = "状态：自动交易启动失败";
            MessageBox.Show(ex.Message, "自动交易", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }

    private async void StopAutoTrade_Click(object sender, RoutedEventArgs e)
    {
        var strategyName = StrategyNameBox.Text.Trim();
        var symbol = SymbolBox.Text.Trim();
        if (string.IsNullOrWhiteSpace(strategyName) || string.IsNullOrWhiteSpace(symbol))
        {
            MessageBox.Show("请填写策略名称和交易对后再停止自动交易。", "停止策略", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }

        var result = MessageBox.Show("停止策略将尝试平掉当前仓位，是否继续？", "停止自动交易", MessageBoxButton.YesNo, MessageBoxImage.Question);
        if (result != MessageBoxResult.Yes)
            return;

        try
        {
            StatusText.Text = $"状态：停止 {strategyName} 自动交易中...";
            await _autoTrade.StopStrategyAsync(strategyName, symbol, flattenPosition: true);
            StatusText.Text = $"状态：策略 {strategyName} 已停止";
        }
        catch (Exception ex)
        {
            StatusText.Text = "状态：停止自动交易失败";
            MessageBox.Show(ex.Message, "停止策略", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }

    private void AutoTrade_StatusChanged(AutoTradeStatus status)
    {
        Dispatcher.Invoke(() =>
        {
            var index = -1;
            ...
            {
                if (string.Equals(_statuses[i].StrategyName, status.StrategyName, StringComparison.OrdinalIgnoreCase) &&
                    string.Equals(_statuses[i].Symbol, status.Symbol, StringComparison.OrdinalIgnoreCase))
                {
                    index = i;
                    break;
                }
