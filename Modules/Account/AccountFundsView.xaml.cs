...
public partial class AccountFundsView : UserControl
{
    ...
    private async void LoadData()
    {
        try
        {
            ...
            if (simAccount != null)
            {
                ...
                string sign = simAccount.TotalPnL >= 0 ? "+" : "";
                if (SimTotalPnLText != null)
                {
                    SimTotalPnLText.Text = $"{sign}{simAccount.TotalPnL:N2}";
                    SimTotalPnLText.Foreground = simAccount.TotalPnL >= 0
    ...
    private async System.Threading.Tasks.Task LoadLiveAccountDataAsync()
    {
        try
        {
            ...
            {
                LogService.Warning("[AccountFundsView] Binance API 凭证未配置，无法加载真实账户数据");

                // 显示未激活状态
                if (LiveNetValueText != null)
                {
                    LiveNetValueText.Text = "0.00 USDT";
                    LiveNetValueText.Foreground = new SolidColorBrush(Color.FromRgb(156, 163, 175));
                }

                if (LiveAvailableText != null)
                {
                    LiveAvailableText.Text = "0.00";
                    LiveAvailableText.Foreground = new SolidColorBrush(Color.FromRgb(156, 163, 175));
                }

                if (LivePositionValueText != null)
                {
                    LivePositionValueText.Text = "0.00";
                    LivePositionValueText.Foreground = new SolidColorBrush(Color.FromRgb(156, 163, 175));
                }
            ...
            if (usdtBalance != null)
            {
                ...
                LogService.Info("[AccountFundsView] Binance 真实账户数据: 净值={NetValue}, 可用={Available}, 持仓={Position}",
                    netValue, available, positionValue);

                // 更新真实账户显示
                if (LiveNetValueText != null)
                {
                    LiveNetValueText.Text = $"{netValue:N2} USDT";
                    LiveNetValueText.Foreground = Brushes.Black;
                }

                if (LiveAvailableText != null)
                {
                    LiveAvailableText.Text = $"{available:N2}";
                    LiveAvailableText.Foreground = Brushes.Black;
                }

                if (LivePositionValueText != null)
                {
                    LivePositionValueText.Text = $"{positionValue:N2}";
                    LivePositionValueText.Foreground = Brushes.Black;
                }
            ...
            {
                LogService.Warning("[AccountFundsView] Binance 账户中没有 USDT 余额");
                ...
                if (LiveAccountStatusBadge != null && LiveAccountStatusBadge.Child is TextBlock statusText)
                {
                    LiveAccountStatusBadge.Background = new SolidColorBrush(Color.FromRgb(251, 191, 36));
                    statusText.Text = "余额为0";
                }

                if (LiveAccountHint != null)
                {
                    LiveAccountHint.Text = "请先向 Binance 合约账户充值 USDT";
                    LiveAccountHint.Foreground = new SolidColorBrush(Color.FromRgb(251, 191, 36));
                }
            }
        }
        catch (Exception ex)
    ...
    private void CheckUpgrade_Click(object sender, RoutedEventArgs e)
    {
        ...
        MessageBox.Show(message, "升级检查", MessageBoxButton.OK,
            upgradeResult.CanUpgrade ? MessageBoxImage.Information : MessageBoxImage.Warning);
    }
        ...
        if (result == MessageBoxResult.Yes)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                if (decimal.TryParse(dialog.InputValue, out decimal initialBalance))
                {
                    ...
                    {
                        MessageBox.Show($"❌ 升级失败: {message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
                else
                {
...
public class InputDialog : Window
{
    ...
    public InputDialog(string prompt, string defaultValue)
    {
        ...
        okButton.Click += (s, e) => { InputValue = textBox.Text; DialogResult = true; };
